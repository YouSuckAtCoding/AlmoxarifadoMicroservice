package infnet.edu.atddd.Domain.Shared;
...
public class ResultRoot {
    ...
    public ResultRoot(boolean isSuccess, Error error) throws Exception {
        ...
        if (!isSuccess && infnet.edu.atddd.Domain.Shared.Error.IsNone(error)) {
            throw new Exception();
        }
        if (isSuccess && !infnet.edu.atddd.Domain.Shared.Error.IsNone(error)) {
            throw new Exception();
    ...
    public static <T> ResultRoot Success() throws Exception{
        return new ResultRoot(true, infnet.edu.atddd.Domain.Shared.Error.None());
    }
